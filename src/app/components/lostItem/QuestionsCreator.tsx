...
const QuestionsCreator: React.FC<{
    ...
    handleChange: (name: string, value: string | Question[]) => void
}> = ({ formData, handleChange }) => {

    const maxNumOfQuestions = 3, maxNumOfAnswers = 3;

    const addQuestion = () => {
        if (formData.questions.length < maxNumOfQuestions) {
    ...
    const addAnswer = (index: number) => {
        if (formData.questions[index].answers.length < maxNumOfAnswers) {
            const updatedQuestions = [...formData.questions];
    ...
    const handleChangeQuestion = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
        ...
        handleChange("questions", formData.questions);
    }
    ...
    return (
        <div className='flex flex-col mx-auto justify-start items-start p-4 border-[1px] border-primary rounded-lg'>
            <strong className='font-semibold pb-12'>כתוב סימנים עבור אימות מול המאבד לגבי הפריט שמצאת: </strong>
            {formData.questions && <div className='flex flex-col w-full gap-10'>
                {[...Array(formData.questions.length)].map((_, questionIndex) => (
                    <div key={questionIndex} className='flex flex-col gap-5'>
                        ...
                        <div className='flex items-center'>
                            {[...Array(formData.questions[questionIndex].answers.length)].map((_, answerIndex) => (
                                <span key={answerIndex} className="flex-1">
                                    <input
                                        type="text"
                                        value={formData.questions[questionIndex].answers[answerIndex]}
                            ...
                            <button type="button" onClick={() => addAnswer(questionIndex)} title='הוסף תשובה'>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={`${formData.questions[questionIndex].answers.length < maxNumOfAnswers ? 'currentColor' : '#B8B8B8'}`} className="size-6">
                ...
                <button
                    type="button"
                    className={`flex justify-center text-sm p-[5px] gap-2 rounded-md ${formData.questions.length < maxNumOfQuestions ? 'bg-primary' : 'bg-[#B8B8B8] cursor-not-allowed'}`}
                    onClick={addQuestion}
                    title='הוסף שאלה'
                >הוסף שאלה
