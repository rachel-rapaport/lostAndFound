...
const NotMineButton = () => {
    ...
    const handleClick = async () => {
        if (currentUser && foundItemToBlock) {
            try {
                const response = await blockItemForUser(currentUser, foundItemToBlock);
                if (response) {
    ...
    return (
        <div>
            <button type='button' className='flex justify-between delete-btn w-[13vw]' onClick={handleClick}>
                זה לא החפץ שלי
