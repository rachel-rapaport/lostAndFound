...
const Chat: React.FC<{ roomId: string }> = ({ roomId }) => {
  ...
  useEffect(() => {
    ...
    onValue(messagesRef, (snapshot) => {
      ...
      setUserOpen(data.users[0]);

      console.log("Data from DB:", data.users[0]); // הדפס את הנתונים ב-console

      if (!(Array.isArray(data) && typeof data === "string")) {
        setMessages(Object.values(data));
      } else {
        setMessages([]); // אם אין נתונים, תעדכן את הסטייט כמערך ריק
      }
    });
  ...
  console.log("after effect chat", messages);
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex gap-x-4 mt-6">
        {userOpen == String(currentUser?._id) && (
